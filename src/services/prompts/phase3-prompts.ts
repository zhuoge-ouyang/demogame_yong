...
type Phase3ReferenceSection = 'entryPrompt' | 'completionFeedback' | 'bosses' | 'levelNodes'
...
export function getPhase3Prompt(continentId: LandingContinentId, moduleKey: Phase3Module): PromptTemplate {
  ...
  const templates: Record<Phase3Module, PromptTemplate> = {
    'entry-prompt': {
      templateId: `phase3-${continentId}-entry-prompt`,
      moduleLabel: `${meta.name} · 进入提示`,
      contextDependencies: ['phase1-summary', `phase2-${continentId}-all`],
      referenceDocuments: [
        { label: `${meta.name} · 进入提示（同步参考稿）`, content: getPhase3Reference(continentId, 'entryPrompt') }
      ],
...
（整体环境氛围、视觉特征、声音特征）`
    ...
    'completion-feedback': {
      templateId: `phase3-${continentId}-completion-feedback`,
      moduleLabel: `${meta.name} · 通关反馈`,
      contextDependencies: ['phase1-summary', `phase2-${continentId}-all`, `phase3-${continentId}-entry-prompt`],
      referenceDocuments: [
        { label: `${meta.name} · 进入提示（同步参考稿）`, content: getPhase3Reference(continentId, 'entryPrompt') },
        { label: `${meta.name} · 通关反馈（同步参考稿）`, content: getPhase3Reference(continentId, 'completionFeedback') }
      ],
...
以此类推...`
    ...
    'level-nodes': {
      templateId: `phase3-${continentId}-level-nodes`,
